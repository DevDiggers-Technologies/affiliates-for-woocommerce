			...
			if ( ! empty( $visits ) ) {
				...
				foreach ( $visits as $visit ) {
					...
					if ( ! empty( $visit[ 'order_id' ] ) ) {
						...
						$order_html = '<a href="' . esc_url( admin_url( 'post.php?post=' . absint( $order->get_id() ) ) . '&action=edit' ) . '"><strong>#' . esc_attr( $order->get_order_number() ) . ' ' . esc_html( $buyer ) . '</strong></a>';
					} else {
						$order_html = 'N/A';
					}
					...
					if ( in_array( $action, [ 'delete' ] ) ) {
						if ( ! empty( $_GET[ 'id' ] ) ) { // WPCS: input var ok.
							if ( is_array( $_GET[ 'id' ] ) ) { // WPCS: input var ok.
								...
								if ( $error ) {
									$message = sprintf( esc_html__( '%d visit(s) not exists.', 'affiliates-for-woocommerce' ), $error );
		...
		public function extra_tablenav( $which ) {
			if ( 'top' === $which ) {
				...
				<div class="alignleft actions bulkactions ddwcaf-bulk-actions">
					...
					<select name="affiliate-id" class="regular-text ddwcaf-affiliate ddfw-users" data-placeholder="<?php esc_attr_e( 'Select Affiliate', 'affiliates-for-woocommerce' ); ?>" data-role="ddwcaf_affiliate">
						<?php
