...
if ( ! class_exists( 'DDWCAF_Payout_Helper' ) ) {
	...
	class DDWCAF_Payout_Helper {
		...
		public function ddwcaf_send_commissions_payout( $commission_ids, $payment_method ) {
			...
			foreach ( $payout_args as $affiliate_id => $payout_arg ) {
				...
				foreach ( $payout_arg[ 'commission_ids' ] as $commission_id ) {
					...
					$this->commission_helper->ddwcaf_update_commission_status( $commission_id, 'pending_payment', true );
				}
			}
		...
		public function ddwcaf_update_payout_status( $payout_id, $status, $no_email = false, $payout = [] ) {
			...
			$payout = apply_filters( 'ddwcaf_process_custom_payout', $payout, $status, $no_email );
		...
		/**
		 * Pay affiliate unpaid amount function
		 *
		 * @param array $args
		 * @param string $reference
		 * @return int|boolean
		 */
		public function ddwcaf_pay_affiliate_unpaid_amount( $args, $reference = 'manually_by_admin' ) {
			extract( $args );
			...
			if ( empty( $total_commission_amount ) ) {
				$total_commission_amount = $this->commission_helper->ddwcaf_get_affiliate_total_pending_commissions_amount( $args );
			}
			...
			$payout_id = $this->ddwcaf_save_payout( $payout_args );

			$payout_args[ 'id' ] = $payout_id;

			$commission_ids = $this->wpdb->get_results( "SELECT commissions.id FROM {$this->wpdb->ddwcaf_commissions} as commissions WHERE 1=1 $conditions", ARRAY_A );

			$commission_ids_query          = '';
			$commissionsmeta_insert_values = '';

			$flag = count( $commission_ids ) - 1;

			foreach ( $commission_ids as $key => $value ) {
				$commission_ids_query          .= "{$value[ 'id' ]}";
				$commissionsmeta_insert_values .= "( {$value[ 'id' ]}, '_ddwcaf_payout_id', $payout_id )";
			...
			$this->wpdb->query( $this->wpdb->prepare( "UPDATE {$this->wpdb->ddwcaf_commissions} SET status=%s WHERE id IN ($commission_ids_query)", 'pending_payment' ) );

			$this->wpdb->query( "INSERT INTO {$this->wpdb->ddwcaf_commissionsmeta} (commission_id, meta_key, meta_value) VALUES $commissionsmeta_insert_values" );
		...
		public function ddwcaf_get_references( $reference = '' ) {
			...
			return ! empty( $references[ $reference ] ) ? $references[ $reference ] : $references;
		}
