...
if ( ! class_exists( 'DDFW_Setup_Wizard' ) ) {
	...
	class DDFW_Setup_Wizard {
		...
		public function enqueue_scripts( $hook ) {
			...
			wp_enqueue_style( 'ddfw-setup-wizard', DDFW_URL . 'assets/css/setup-wizard.css', [], filemtime(DDFW_FILE . 'assets/css/setup-wizard.css') );
			wp_enqueue_script( 'ddfw-setup-wizard', DDFW_URL . 'assets/js/setup-wizard.js', [], filemtime(DDFW_FILE . 'assets/js/setup-wizard.js'), true );
		...
		public function ajax_save_wizard_step() {
			...
			if ( empty( $plugin_slug ) || empty( $step_id ) ) {
				wp_send_json_error( [ 'message' => esc_html__( 'Invalid request.', 'loyaltyx-points-and-rewards-for-woocommerce' ) ] );
			}

			if ( ! isset( $this->args['steps'][ $step_id ] ) ) {
				wp_send_json_error( [ 'message' => esc_html__( 'Step not found.', 'loyaltyx-points-and-rewards-for-woocommerce' ) ] );
			}
		...
		public function ready_view( $step = [] ) {
			$title = $step['ready_title'] ?? esc_html__( 'Congratulations! You are all set.', 'loyaltyx-points-and-rewards-for-woocommerce' );
			$desc  = $step['ready_description'] ?? esc_html__( 'You can now start using the plugin and configure more advanced settings from the dashboard.', 'loyaltyx-points-and-rewards-for-woocommerce' );
			?>
				...
				<div class="ddfw-success-icon-wrap">
					<div style="background:#eef2ff; border-radius:50%; width:80px; height:80px; display:flex; align-items:center; justify-content:center;">
						<svg width="40" height="40" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
							<path d="M5 13L9 17L19 7" stroke="#2563eb" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>
						</svg>
					</div>
				</div>
