		...
		if ( ! empty( $license_activated ) ) {
			...
			if ( ! is_wp_error( $response ) ) {
				...
				if ( ! empty( $response_data ) && 
					...
					wp_die( sprintf( esc_html__( "License Deactivated for %s", 'affiliates-for-woocommerce' ), esc_html( $prefix ) ), 'License Deactivated', [ 'response' => 200 ] );
				}
			}
		}

		// If we reach here, no matching license was found or deactivation failed
		wp_die( esc_html__( 'License not found or deactivation failed', 'affiliates-for-woocommerce' ), esc_html__( 'Deactivation Failed', 'affiliates-for-woocommerce' ), [ 'response' => 404 ] );
	}
}
